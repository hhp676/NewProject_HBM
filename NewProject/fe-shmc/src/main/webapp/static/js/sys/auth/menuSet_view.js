...
sysMenuSet._confirm = function(rowId) {
	...
	data.modelId = rowId;
	/*获取编辑器,读取编辑器值*/
	var editor = $('#menu_grid').treegrid('getEditor', {id:rowId,field:'isBsTopMenu'});
	data.isBsTopMenu = $(editor.target).combobox('getValue');
	/*获取编辑器,读取编辑器值*/
	editor = $('#menu_grid').treegrid('getEditor', {id:rowId,field:'isBsSubMenu'});
	data.isBsSubMenu = $(editor.target).combobox('getValue');
